...
def render():
    ...
    if st.button("🔍 분석 실행", type="primary", use_container_width=True, key="listed_run"):
        ...
        pre_debug = ["[listed.py] 주가 수집 블록 진입"]
        st.session_state.stock_debug = pre_debug
        try:
            with st.spinner("주가 정보 조회 중..."):
                stock_info = fetch_stock_info(corp_name)
            st.session_state.stock_debug.append("[listed.py] fetch_stock_info 호출 완료")
        except Exception as ex:
            import traceback
            st.session_state.stock_debug.append(f"[listed.py] fetch_stock_info 호출 중 예외: {ex}")
            st.session_state.stock_debug.append(traceback.format_exc())
            stock_info = None
